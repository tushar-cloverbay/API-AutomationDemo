...
public class Scenario_4_7 extends Base{
	...
	public void submitPrepare_enUSSMS2FactorAuth(String version) throws IOException {
		...
		{
			payload = JsonUtils.payloadGenerator("Inputs\\"+Constants.ENV+"\\Scenario_4\\preparedEID-SMS2FactorAuth-"+version+".json");
		}else
		{
			payload = JsonUtils.payloadGenerator("Inputs\\"+Constants.ENV+"\\Scenario_4\\preparedEID-SMS2FactorAuth.json");
		}
	...
	public void submit_enUSSMS2FactorAuth(String version) throws IOException {
		extentTest.log(LogStatus.PASS, "Test Description : " + "Scenario_4_7 : Test for Submit for en-US-SMS signing with two factor authentication");
		apiVersion = version;
		String token =TestUtils.getToken(version);
		String preparedEID;
		if(version.equals("3.0")||version.equals("3.1")||version.equals("3.2"))
		{
			preparedEID = TestUtils.getPreparedEnvelopeID(version, "Scenario_4\\preparedEID-SMS2FactorAuth-"+version+".json",token);
		}else
		{
			preparedEID = TestUtils.getPreparedEnvelopeID(version, "Scenario_4\\preparedEID-SMS2FactorAuth.json",token);
		}
